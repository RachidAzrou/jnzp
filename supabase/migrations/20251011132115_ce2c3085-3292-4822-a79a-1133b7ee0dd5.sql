-- Fix fn_register_org_with_contact: change p_user_id parameter type to UUID
...
create or replace function public.fn_register_org_with_contact(
  ...
  p_email text,
  p_phone text,
  p_user_id uuid,  -- Changed from default null to required UUID
...
declare
  v_org_id  uuid;
  v_user_id uuid := p_user_id;  -- Use the passed user_id directly
begin
  ...
  -- Koppel user ↔ org met EXACTE user_id die is meegegeven
