-- Fix fn_register_org_with_contact: use only existing columns
...
create or replace function public.fn_register_org_with_contact(
  ...
  p_set_active boolean default false  -- behouden voor compat, maar niet gebruikt
) returns uuid
language plpgsql
security definer
...
begin
  ...
  -- Koppel user ↔ org (let op kolomnaam)
