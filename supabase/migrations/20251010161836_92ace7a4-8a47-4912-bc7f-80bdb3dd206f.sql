-- Drop bestaande functie versies
...
-- Recreate met UPSERT strategie (voorkomt race conditions)
...
BEGIN
  ...
  -- Map org_type naar enum en role
  CASE p_org_type
  ...
  -- DUURZAME FIX: Verwijder ALLEEN conflicterende roles (minimaliseert trigger activaties)
  DELETE FROM user_roles 
  WHERE user_id = v_user_id 
    AND role IN (v_role, 'org_admin'::app_role);

  -- UPSERT primary role (atomisch, voorkomt race conditions)
  ...
  -- UPSERT org_admin role voor professional organizations
  ...
  -- Update profile (zonder organization_id column)
  UPDATE profiles
  SET
