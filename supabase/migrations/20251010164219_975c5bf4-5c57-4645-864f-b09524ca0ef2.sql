-- Harden de functie met expliciete validatie en duidelijke foutmeldingen
...
BEGIN
  -- Expliciete validatie van verplichte velden
  IF NULLIF(TRIM(p_org_type), '') IS NULL THEN
    RAISE EXCEPTION 'Missing required param: org_type';
  END IF;
  
  IF NULLIF(TRIM(p_company_name), '') IS NULL THEN
    RAISE EXCEPTION 'Missing required param: company_name';
  END IF;
  
  IF NULLIF(TRIM(p_email), '') IS NULL THEN
    RAISE EXCEPTION 'Missing required param: email';
  END IF;
  ...
  IF v_final_user_id IS NULL THEN
    RAISE EXCEPTION 'Missing required param: user_id (no authenticated user)';
  END IF;

  -- Map org_type to enum and role met validatie
  CASE UPPER(TRIM(p_org_type))
    ...
    ELSE 
      RAISE EXCEPTION 'Invalid org_type: % (must be FUNERAL_DIRECTOR, MOSQUE, INSURER, MORTUARIUM, or FAMILY)', p_org_type;
  END CASE;
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE EXCEPTION 'fn_register_org_with_contact failed: % (SQLSTATE: %)', SQLERRM, SQLSTATE;
END;
$$;