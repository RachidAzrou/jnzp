...
BEGIN
  ...
  -- DEBUG: Log alle user_id sources
  RAISE NOTICE 'DEBUG user_id resolution: p_user_id=%, auth.uid()=%, v_user_id=%', 
    p_user_id, auth.uid(), v_user_id;
  ...
  -- DEBUG: Log before user_roles insert
  RAISE NOTICE 'DEBUG before user_roles: v_user_id=%, v_role=%, v_org_id=%', 
    v_user_id, v_role, v_org_id;

  -- 4) Insert primary role - EXPLICIET v_user_id gebruiken
  INSERT INTO user_roles (user_id, role, organization_id, is_admin, scope)
  VALUES (v_user_id, v_role, v_org_id, true, 'ORG')
  ON CONFLICT (user_id, role) DO NOTHING;

  -- 5) Insert org_admin role
