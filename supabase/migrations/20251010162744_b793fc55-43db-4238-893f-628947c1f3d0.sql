-- TIJDELIJK: Schakel audit trigger VOLLEDIG UIT om te testen
-- Als dit het probleem oplost, moeten we de trigger herschrijven
ALTER TABLE public.user_roles DISABLE TRIGGER audit_role_changes_trigger;

-- Voeg debug logging toe aan de functie
...
BEGIN
  -- Get user_id from parameter or auth context
  v_user_id := COALESCE(p_user_id, auth.uid());
  
  RAISE NOTICE 'DEBUG: v_user_id at start = %', v_user_id;
  ...
  RAISE NOTICE 'DEBUG: v_user_id before first INSERT = %', v_user_id;
  ...
  RAISE NOTICE 'DEBUG: v_user_id after first INSERT = %', v_user_id;

  -- UPSERT org_admin role
  IF v_org_type_enum IN ('FUNERAL_DIRECTOR', 'MOSQUE', 'INSURER', 'MORTUARIUM') THEN
    RAISE NOTICE 'DEBUG: v_user_id before second INSERT = %', v_user_id;
    ...
    RAISE NOTICE 'DEBUG: v_user_id after second INSERT = %', v_user_id;
  END IF;
