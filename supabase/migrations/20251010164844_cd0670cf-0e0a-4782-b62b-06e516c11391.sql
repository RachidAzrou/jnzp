-- Verbeter exception handling met detail info
...
BEGIN
  ...
  IF v_final_user_id IS NULL THEN
    RAISE EXCEPTION 'Missing required param: p_user_id (no authenticated user)';
  END IF;
...
EXCEPTION
  WHEN not_null_violation THEN
    ...
    RAISE EXCEPTION 'NOT NULL violation in fn_register_org_with_contact: % | Detail: %', SQLERRM, v_detail;
    
  WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS v_detail = PG_EXCEPTION_DETAIL;
    RAISE EXCEPTION 'fn_register_org_with_contact failed: % | SQLSTATE: % | Detail: %', SQLERRM, SQLSTATE, v_detail;
END;
$$;