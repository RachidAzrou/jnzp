-- Fix ON CONFLICT clause in fn_register_org_with_contact
-- The issue: ON CONFLICT was only matching on (user_id, role) but when user_id was NULL in existing rows,
-- it created a new row instead of updating, causing the not-null constraint violation
...
BEGIN
  ...
  -- Clean up any orphaned roles for this user first
  DELETE FROM user_roles WHERE user_id = p_user_id;

  -- Insert primary role (no conflict possible after DELETE)
